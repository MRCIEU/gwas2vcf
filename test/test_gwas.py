...
def test_pvalues_precision_with_PvalueHandler():
    ...
    for p_value_string in p_value_strings:
        gwas_list.append(Gwas('test', 1, 'A', 'T', None, None,
                              p_value_handler.neg_log_of_decimal(p_value_handler.parse_string(p_value_string)),
...
def test_cyvcf2_parse():
    dbsnp = VCF(os.path.join(os.path.dirname(__file__), "dbsnp.vcf.gz"))
    dbsnpid = None
    for v in dbsnp(region='{}:{}-{}'.format("test", 1, 1)):
        dbsnpid = v.ID
        break
    assert dbsnpid == "rs1234"
    dbsnp.close()
